...
import { getAllResolvedModels, resolveModel, type ProviderConfig, type McpServerConfig, type Session } from '../../lib/storage';
              ...
              {isSystem ? (
                ...
                <div className="flex flex-col gap-1.5 max-w-full">
                  ...
                  <div className="relative ai-markdown px-3 py-2 rounded-[5px_18px_18px_18px] bg-muted text-[12.5px] leading-relaxed break-words overflow-hidden">
                    ...
                    dangerouslySetInnerHTML={{ __html: marked.parse(desensitize(m.text)) as string }}
                    onClick={(e) => {
              ...
              .map((s) => {
                ...
                return (
                  <button key={s.id} onMouseDown={(e) => {
                    ...
                    <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.messages.find((m) => m.role === 'user')?.text.slice(0, 40) ?? s.id}</span>
                    <span style={{ fontSize: 10, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{stepCount} 步骤</span>
                  </button>
                );
              })}
            {sessions.filter((s) => s.messages.some((m) => m.toolCall)).length === 0 && (
              <div style={{ padding: '6px 10px', fontSize: 11, color: 'var(--text-muted)' }}>暂无含工具调用的会话</div>
            )}
