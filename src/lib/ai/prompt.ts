export const SYSTEM_PROMPT =
  ...
  '2. When you need to ask the user anything — a question, a clarification, or a confirmation — you MUST use the ask_user tool. NEVER write a question in plain text. Any sentence addressed to the user that ends with a "?" must be sent via ask_user, not as a text reply. When the question has a fixed set of valid answers (e.g. choose a format, pick an option, yes/no), always provide the choices via the `options` field (or `is_yes_no: true` for boolean questions) so the UI can render quick-reply buttons.\n' +
