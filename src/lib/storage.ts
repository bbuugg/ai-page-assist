...
// ---- Preview HTML ----
export async function savePreviewHtml(html: string): Promise<void> {
  return new Promise((resolve) => chrome.storage.local.set({ previewHtml: html }, resolve));
}

export async function loadPreviewHtml(): Promise<string> {
  return new Promise((resolve) => {
    chrome.storage.local.get(['previewHtml'], (result) => {
      resolve(result.previewHtml ?? '');
    });
  });
}
