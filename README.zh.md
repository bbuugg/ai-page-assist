...
- **代码块「发送到预览」按钮** — AI 回复中每个 HTML 代码块右上角的快捷按钮
- **实时同步** — AI 流式生成时，预览页自动跟随更新（如已打开）
- 通信机制：通过 `chrome.storage.local` 的 `previewHtml` 键写入；预览页监听 `chrome.storage.onChanged` 自动刷新
...
preview.html         # HTML 预览页入口
  preview/
    PreviewApp.tsx   # 左侧代码编辑器 + 右侧 iframe 实时渲染
manifest.json        # Chrome 扩展清单
vite.config.ts       # 构建配置（4 个入口 → plugin/）
```
